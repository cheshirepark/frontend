...
export default function News() {
  ...
  const [corporateNewsStream, setCorporateNewsStream] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function pullFullHistoryIndex() {
      try {
        ...
        const dynamicPayload = data.map((item) => {
        ...
        console.error(
          "Failed to collect historical regulatory stream records:",
          err,
        );
      } finally {
        setLoading(false);
  ...
  return (
    <div className="w-full bg-mining-alabaster font-sans">
      <section
        ...
        style={{
          backgroundImage: `url('https://images.unsplash.com/photo-1504711434969-e33886168f5c?q=80&w=1600&auto=format&fit=crop')`,
        }}
      >
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-12 py-16">
          ...
          <div className="flex items-center space-x-1 overflow-x-auto pb-1 md:pb-0">
            ...
            {yearTimelineOptions.map((year) => (
              <button
                ...
                className={`px-4 py-2 text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer rounded-none border ${selectedYear === year ? "bg-mining-obsidian border-mining-obsidian text-white" : "bg-slate-50 border-slate-200 text-slate-600 hover:border-slate-300 hover:bg-slate-100"}`}
              >
                {year}
              </button>
            ))}
        ...
        {loading ? (
          <div className="text-center py-24 text-xs font-mono tracking-widest text-slate-400">
            ...
            {filteredNewsItems.map((post) => (
              ...
              >
                ...
                <div className="pt-1 md:pt-4 self-end md:self-auto shrink-0">
                  <a
                    href={`/news/${post.slug}`}
                    className="text-[10px] uppercase tracking-widest font-bold text-mining-obsidian group-hover:text-mining-gold inline-flex items-center gap-1.5"
                  >
                    View Release{" "}
